...
def main():
    ...
    # 1. Download Metadata Datasets first (districts.csv, police_stations.csv, crime_categories.csv, crime_subcategories.csv, officers.csv)
    metadata_files = [
        "districts.csv",
        "police_stations.csv",
    ...
    # 2. Download FIR archives (FIR1.csv .. FIR9.csv)
    for n in range(1, 10):
        name = f"FIR{n}.csv"
        key = f"archive/{name}"
