...
def init_catalyst():
    ...
    """
    import os
    logger.info(f"CATALYST_AUTH: {os.environ.get('CATALYST_AUTH')}")
    logger.info(f"CATALYST_OPTIONS: {os.environ.get('CATALYST_OPTIONS')}")
    try:
...
def run_migration():
    ...
    # List files
    csv_files = list(CSV_TO_TABLE_MAP.keys())
    logger.info(f"Found {len(csv_files)} CSV files to process based on mapping.")
